...
export default function ExchangeRateWidget({
  ...
  useEffect(() => {
    if (
      ...
      !selectedCurrencies.includes(settings.reportCurrency) &&
      settings.reportCurrency !== settings.currency
    ) {
                ...
                return (
                  <div key={code} className='flex items-center justify-between'>
                    ...
                    <span
                      ...
                      title={`1 ${settings.currency} = ${rate ? rate.toFixed(4) : "—"} ${code}`}>
      ...
      {loading && !rates ? (
        ...
        <div className='p-4 bg-amber-50 rounded-lg border border-amber-100'>
          ...
          <p className='mt-1 text-sm text-amber-600'>
            Using cached or fallback rates
          </p>
        </div>
      ) : (
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
          {SUPPORTED_CURRENCIES.filter((c) => c.code !== settings.currency)
            .slice(0, 8)
            .map((currency) => {
              const rate = rates?.rates[currency.code];
              const isReportCurrency =
                currency.code === settings.reportCurrency;

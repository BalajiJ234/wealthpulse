...
interface BulkImportProps {
  feature: 'expenses' | 'friends' | 'income' | 'goals' | 'commitments' | 'debts'
...
const templates = {
  expenses: {
    ...
    sample: [
      { amount: 25.50, description: 'Lunch at restaurant', category: 'Food & Dining', date: '2025-01-10', currency: 'AED' },
      { amount: 1200, description: 'Monthly rent', category: 'Housing', date: '2025-01-01', currency: 'AED' },
      { amount: 45, description: 'Gas station', category: 'Transportation', date: '2025-01-09', currency: 'AED' }
    ]
  },
  income: {
  ...
  goals: {
    headers: ['title', 'targetAmount', 'currentAmount', 'currency', 'targetDate'],
    sample: [
      { title: 'Emergency Fund', targetAmount: 30000, currentAmount: 5000, currency: 'AED', targetDate: '2026-12-31' },
      { title: 'Vacation Fund', targetAmount: 10000, currentAmount: 0, currency: 'AED', targetDate: '2026-06-30' },
      { title: 'New Laptop', targetAmount: 8000, currentAmount: 2000, currency: 'AED', targetDate: '2026-03-01' }
    ]
  },
  commitments: {
    headers: ['title', 'amount', 'currency', 'dueDate', 'priority', 'recurringType', 'category'],
    sample: [
      { title: 'Rent', amount: 5500, currency: 'AED', dueDate: '2026-06-01', priority: 'HIGH', recurringType: 'MONTHLY', category: 'Housing' },
      { title: 'Car Insurance', amount: 2400, currency: 'AED', dueDate: '2026-07-15', priority: 'MEDIUM', recurringType: 'YEARLY', category: 'Insurance' },
      { title: 'School Fees', amount: 15000, currency: 'AED', dueDate: '2026-08-01', priority: 'HIGH', recurringType: 'YEARLY', category: 'Education' }
    ]
  },
  debts: {
    headers: ['debtName', 'lenderName', 'totalAmount', 'remainingBalance', 'monthlyPayment', 'interestRate', 'currency', 'debtType'],
    sample: [
      { debtName: 'Car Loan', lenderName: 'ADCB', totalAmount: 60000, remainingBalance: 45000, monthlyPayment: 1500, interestRate: 5.5, currency: 'AED', debtType: 'loan' },
      { debtName: 'Credit Card', lenderName: 'Emirates NBD', totalAmount: 20000, remainingBalance: 8000, monthlyPayment: 800, interestRate: 24, currency: 'AED', debtType: 'credit_card' }
    ]
  }
}
        ...
        switch (feature) {
          ...
          case 'goals':
            ...
            if (!row.targetAmount || isNaN(parseFloat(row.targetAmount))) {
              errors.push(`Row ${rowNum}: Invalid targetAmount "${row.targetAmount}"`)
              return
            }
            valid.push({
              id: Date.now().toString() + Math.random(),
          ...
          case 'commitments':
            ...
            valid.push({
              ...
              priority: (['HIGH', 'MEDIUM', 'LOW'].includes(row.priority?.toUpperCase()) ? row.priority.toUpperCase() : 'MEDIUM'),
              recurringType: (['NONE', 'MONTHLY', 'YEARLY'].includes(row.recurringType?.toUpperCase()) ? row.recurringType.toUpperCase() : 'NONE'),
              status: 'UPCOMING',
            })
            break
          ...
          case 'debts':
            ...
            if (!row.remainingBalance || isNaN(parseFloat(row.remainingBalance))) {

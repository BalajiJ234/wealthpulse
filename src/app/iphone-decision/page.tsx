...
import { api, IPhoneDecisionResult } from "@/lib/api";
import { Smartphone, CheckCircle2, AlertTriangle, XCircle, Clock } from "lucide-react";
...
const DECISION_CONFIG: Record<string, { label: string; color: string; icon: typeof CheckCircle2; bg: string }> = {
  ...
  BUY_LOWER_MODEL: { label: "Consider Lower Model", color: "text-blue-700", bg: "bg-blue-50 border-blue-200", icon: AlertTriangle },
  AVOID_FOR_NOW: { label: "Avoid for Now", color: "text-red-700", bg: "bg-red-50 border-red-200", icon: XCircle },
};

export default function IPhoneDecisionPage() {
  const [price, setPrice] = useState("5000");
  const [emiMonths, setEmiMonths] = useState("12");
  const [result, setResult] = useState<IPhoneDecisionResult | null>(null);
  ...
  const handleCheck = async () => {
    ...
    setError(null);
    try {
      const r = await api.getIPhoneDecision(Number(price), Number(emiMonths));
      setResult(r);
    } catch (e: unknown) {
  ...
  return (
    <main className="max-w-xl mx-auto px-4 py-8 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900">iPhone Purchase Decision</h1>
      </div>
      <p className="text-sm text-gray-500">
        Enter the iPhone price and your preferred EMI months. The engine analyses your current financial health and gives a recommendation.
      </p>

      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs text-gray-600 mb-1">iPhone Price (AED)</label>
            <input
              type="number"
              value={price}
        ...
        <div className={`rounded-xl border p-6 space-y-4 ${cfg.bg}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Decision score: {result.score}/100</p>
            </div>
          </div>
          <div className="space-y-2">
            <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Reasons</p>
          ...
          <div className="pt-3 border-t border-gray-200 text-xs text-gray-500">
            Input: AED {result.input.iphonePrice} over {result.input.emiMonths} months
      ...
      <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 text-xs text-gray-500 space-y-1">
        ...
        <p>• iPhone EMI ≤ 10–15% of monthly income</p>
